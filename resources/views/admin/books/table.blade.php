<!-- Desktop Table -->
<div class="hidden md:block bg-white rounded-2xl shadow-md overflow-hidden border border-gray-100 animate-fade-in">
    <div class="overflow-x-auto">
        <table class="w-full">
            <thead class="bg-gradient-to-r from-white-300 to-white-700 text-blue">
                <tr>
...
<div class="md:hidden space-y-4">
    @forelse($books as $index => $book)
    <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-4 animate-fade-in">
        <!-- Header dengan Judul & Icon -->
        <div class="flex items-start justify-between mb-3">
            <div class="flex items-center flex-1">
                ...
                <div class="flex-1 min-w-0">
                    <h3 class="text-sm font-bold text-gray-900 truncate">{{ $book->judul }}</h3>
                    <p class="text-xs text-gray-500">{{ $book->created_at->format('d M Y') }}</p>
                </div>
            </div>
        ...
        <div class="space-y-2 mb-4">
            <div class="flex items-center text-xs">
                <span class="text-gray-500 w-24 flex-shrink-0">Kode</span>
                <span class="font-semibold text-gray-900">{{ $book->code }}</span>
            </div>
            <div class="flex items-center text-xs">
                <span class="text-gray-500 w-24 flex-shrink-0">Penulis</span>
                <span class="text-gray-700">{{ $book->penulis }}</span>
            </div>
            <div class="flex items-center text-xs">
                <span class="text-gray-500 w-24 flex-shrink-0">Kategori</span>
                <span class="{{ $book->Category->color }} text-xs font-medium px-2 py-0.5 rounded-full">
                    {{ $book->Category->name }}
                </span>
            </div>
            <div class="flex items-center text-xs">
                <span class="text-gray-500 w-24 flex-shrink-0">Tahun Terbit</span>
                <span class="text-gray-700">{{ $book->tahun_terbit ?? 'N/A' }}</span>
            </div>
        </div>

        <!-- Action Buttons -->
        <div class="flex items-center gap-2 pt-3 border-t border-gray-100">
            <button @click="openEditModal({id: {{ $book->id }}, judul: @js($book->judul), code: @js($book->code), penulis: @js($book->penulis), tahun_terbit: @js($book->tahun_terbit), category_id: {{ $book->category_id }}, body: @js($book->body)})" class="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors text-sm font-medium">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                </svg>
                Edit
            </button>
            <a href="/books/{{ $book->code }}" class="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-green-600 bg-green-50 hover:bg-green-100 rounded-lg transition-colors text-sm font-medium">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                </svg>
    ...
    <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-8">
        <div class="flex flex-col items-center">
            ...
            <p class="text-gray-500 text-base font-medium">Belum ada buku</p>
            <p class="text-gray-400 text-sm text-center">Klik tombol "Tambah Buku" untuk menambahkan buku baru</p>
        </div>
    </div>
    @endforelse
</div>
